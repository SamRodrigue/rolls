const s = sketch => {  
            ...
            for (let j = -view.brush.val / 2; j < view.brush.val / 2; j += map.texture.scale) {
              ...
              if (d < dLimit) {
                ...
                if (map.texture.val[x][y] === 0) {
                  ...
                  const tx = x % textures.images[map.texture.val[x][y]].width;
                  const ty = y % textures.images[map.texture.val[x][y]].height;
                  const tex = textures.images[map.texture.val[x][y]].get(tx, ty);
            ...
            for (let j = -view.brush.val / 2; j < view.brush.val / 2; j += map.fog.scale) {
              ...
              if (d < dLimit) {
                ...
                if (map.fog.val[x][y] === 0) {
                  ...
                  const tx = x % textures.images[textures.FOG].width;
                  const ty = y % textures.images[textures.FOG].height;
                  const tex = textures.images[textures.FOG].get(tx, ty);
  ...
  class Wall {
    ...
    draw() {
      ...
      if (this.type === walls.NONE) { // Solid wall
        ...
        if (this.image === null) {
          const tex = walls.images[this.type];
  
          const mag = Math.round(Math.sqrt(par.x * par.x + par.y * par.y) / map.wall.scale + tex.height);
    ...
    drawToMouse(x, y) {
      ...
      if (this.type === walls.NONE) { // Solid wall
        ...
        if (par.x !== 0 || par.y !== 0) {
          const tex = walls.images[this.type];
          const tex_width = tex.width * map.wall.scale;
  
          const mag = Math.round(Math.sqrt(par.x * par.x + par.y * par.y) / map.wall.scale) + tex.height;
          const segments = Math.floor(mag / tex.width);
          const remain = mag % tex.width;
  
          // Draw remaining wall as a solid line
          // Drawn first to appear below full wall segments
          if (remain) {
            const scale_mag = Math.sqrt(par.x * par.x + par.y * par.y);
            const per = {
  ...
  sketch.load_all = data => {
    ...
    sketch.load_type('walls', { walls: data.walls });
    sketch.load_type('entities', { entities: data.entities });
    sketch.load_type('assets', { assets: data.assets });
    sketch.load_type('texture', { texture: data.texture });
    sketch.load_type('fog', { fog: data.fog });
  };
  ...
  sketch.save_all = () => {
    ...
    const data = {
      share:    map.share,
      walls:    sketch.save_type('walls').walls,
      entities: sketch.save_type('entities').entities,
      assets:   sketch.save_type('assets').assets,
      texture:  sketch.save_type('texture').texture,
      fog:      sketch.save_type('fog').fog
    };
  
    return data;
  };
  
  sketch.save_type = type => {
    const out = [];
    switch (type) {
      case 'walls':
        for (w of map.walls) {
      ...
      case 'entities':
        for (e of map.entities) {
          if (user.role === 'admin' ||
            (user.role === 'user' && user.id === e.user.id)) {
            out.push(e.data());
          }
        }
        return { entities: out };
  ...
  sketch.load_type = (type, data) => {
    switch (type) {
      ...
      case 'texture':
        const texture = decompress_texture(data.texture);
        map.texture.image.loadPixels();
        ...
        for (let i = 0; i < map.texture.width; ++i) {
          for (let j = 0; j < map.texture.height; ++j) {
            ...
            if (curr === 0) {
              ...
              const ti = i % textures.images[curr].width;
              const tj = j % textures.images[curr].height;
              const tex = textures.images[curr].get(ti, tj);
      ...
      case 'fog':
        const fog = decompress_texture(data.fog);
        map.fog.image.loadPixels();
        ...
        for (let i = 0; i < map.fog.width; ++i) {
          for (let j = 0; j < map.fog.height; ++j) {
            ...
            if (curr === 0) {
              ...
              const ti = i % textures.images[textures.FOG].width;
              const tj = j % textures.images[textures.FOG].height;
              const tex = textures.images[textures.FOG].get(ti, tj);
              if (user.role !== 'admin') tex[3] = 255;
  ...
  sketch.mouseDragged = () => {
    ...
    const current_time = new Date().getTime();
    const dt = current_time - mode.dragging.time;
  ...
  function draw_lines() {
    ...
    // Per user lines
    Object.keys(map.lines).forEach(a_user_id => {
      ...
      map.lines[a_user_id].some((marker, i, iarr) => {
    ...
    if (mode.cursor.val === cursors.MOVE ||
      ...
      for (entity of map.entities) {
        ...
        if (entity.contains(ex, ey)) {
          const t_size = 16;
          const e_width = Math.min(256, sketch.width / 3);
          const e_height = e_width + 2.75 * t_size;
          const scale = e_width / 6 * map.entity.scale;
